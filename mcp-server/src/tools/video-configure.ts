...
export function registerVideoConfigure(server: McpServer): void {
  server.tool(
    ...
    "Configure video perception preferences (backend, resolution, fps, whisper model, etc.)",
    {
      ...
      whisper_threads: z.union([z.number().int().positive(), z.literal("auto")]).optional(),
      whisper_timeout_multiplier: z.number().positive().optional(),
      whisper_timeout_min_seconds: z.number().min(60).optional(),
